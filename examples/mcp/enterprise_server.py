...
for mw in [
    AuthMiddleware(auth),
    LoggingMiddleware(),
    RateLimitMiddleware(rate_per_minute=120),
...
async def search_employees(query: str) -> str:
    ...
    matches = [
        e
        for e in EMPLOYEES
        if query.lower() in e["name"].lower() or query.lower() in e["role"].lower()
    ]
    if not matches:
...
async def list_transactions(category: str = "", limit: int = 10) -> str:
    ...
    rows = rows[:limit]
    lines = [
        f"  {t['id']} | {t['date']} | ${t['amount']:,} | {t['category']} | {t['vendor']}"
        for t in rows
    ]
...
async def get_budget_vs_actual(department: str = "") -> str:
    ...
    data = {
        "Engineering": (800000, 788000),
        "Sales": (400000, 421000),
        "Marketing": (200000, 195000),
        "Operations": (150000, 148000),
        "HR": (100000, 92000),
        "Finance": (80000, 75000),
    }
    ...
    lines = [
        f"  {d}: Budget ${b:,} | Actual ${a:,} | {'+' if b > a else ''}{(b - a) / b * 100:.1f}%"
        for d, (b, a) in data.items()
    ]
...
async def list_products(warehouse: str = "") -> str:
    ...
    lines = [
        f"  {i['sku']} | {i['name']} | Qty: {i['qty']} | ${i['price']} | {i['warehouse']}"
        for i in rows
    ]
...
async def get_inventory_value() -> str:
    ...
    lines = [
        f"  {i['name']}: {i['qty']} x ${i['price']} = ${i['qty'] * i['price']:,.2f}"
        for i in INVENTORY
    ]
...
async def list_tickets(status: str = "", priority: str = "") -> str:
    ...
    lines = [
        f"  {t['id']} [{t['priority']}] {t['subject']} — {t['customer']} ({t['status']})"
        for t in rows
    ]
...
async def ticket_stats() -> str:
    ...
    return (
        f"By Status: {dict(by_status)} | By Priority: {dict(by_priority)} | Total: {len(TICKETS)}"
    )
...
if __name__ == "__main__":
    ...
    print("Roles: viewer (10 tools), analyst (25 tools), admin (all 30)")
    print()
